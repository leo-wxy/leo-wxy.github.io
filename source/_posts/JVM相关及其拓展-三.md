...
更完整地说，JVM里一个类的“身份”通常由两部分共同决定：

- 类的全限定名
- 定义它的类加载器

因此即使字节码内容完全一样，只要由不同的类加载器定义，在JVM看来它们就是两个不同的类型，这也是很多模块隔离、插件化机制能够成立的基础。
...
补充：这里是偏 JDK 8 时代的常见描述。在较新的JDK版本（尤其 JDK 9+ 模块化之后）里，类加载器的实现细节和职责边界已经有所调整，回答问题时更建议区分“启动类加载器 / 平台类加载器 / 应用类加载器”这类现代口径，而不要把`ExtClassLoader`当作所有版本下都完全一致的事实。
...
这里有一个很容易混淆的点：双亲委派主要发生在`loadClass()`这一层，而不是一开始就进入`findClass()`。

- `loadClass()`负责整体加载流程控制：先查是否已加载，再委派父加载器，最后才决定是否由自己尝试加载。
- `findClass()`更像当前加载器真正“自己去找字节码并定义类”的实现入口。

所以通常说“先委派父加载器”，本质上说的是`loadClass()`的默认实现策略。
...
进一步说，很多时候“破坏双亲委派”并不是为了故意和JVM对着干，而是为了满足动态性或隔离性的需求。常见场景包括：

- 容器或模块化系统需要让不同模块加载各自版本的同名类
- 热部署、热替换需要重新加载实现类
- 插件化/热修复需要在运行时引入新的类定义

所以这类做法通常是“带着明确目标去打破默认策略”，而不是日常业务代码的常规做法。
...
- 实现自定义的ClassLoader
  ...
  一般来说，自定义类加载器更推荐优先重写`findClass()`，而不是直接重写`loadClass()`。这样做的好处是：

  - 可以保留`loadClass()`默认实现中的双亲委派逻辑
  - 只在父加载器无法完成加载时，才由当前加载器真正去查找并定义类

  只有在明确需要打破双亲委派时，才通常会进一步介入`loadClass()`本身的流程控制。
  ...
  这里的`defineClass()`是关键步骤：它负责把读取到的字节数组真正转换成JVM可识别、可使用的`Class`对象。
