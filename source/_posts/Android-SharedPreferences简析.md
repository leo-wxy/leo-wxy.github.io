...
- 同一进程内，`apply()`后新值通常可立即从内存读到。
- 跨进程场景不保证“立即一致”，即使开启`MODE_MULTI_PROCESS`也只是尽量重载文件。
- 需要跨进程强一致时，应通过`ContentProvider/数据库/MMKV(DataStore)`等方案统一读写入口。
...
- 探测粒度受文件时间戳/大小变化影响，存在时序窗口。
- 高频并发写入下，进程间仍可能看到短暂不一致。
- 因此它更偏兼容兜底，不适合作为多进程一致性的主方案。
...
补充：触发卡顿的关键不是`apply()`本身，而是“生命周期关键点上的集中等待”。

- 前台频繁`apply()`看似快，但任务会积压到`waitToFinish()`统一清算。
- 一旦XML较大或设备IO抖动，主线程等待时间会迅速放大。
- 排查时建议同时观察：写入频率、单次文件大小、`onStop`阶段阻塞时长。
...
- 对高频更新字段先做内存聚合（去抖/节流），再按窗口批量落盘。
- 将“统计类、埋点类、小步快写”与“关键配置写入”拆分，避免互相放大延迟。
