...
`Future`本质上是一个“异步结果句柄”：

- `get()`会阻塞当前线程直到结果就绪
- `get(timeout, unit)`可以给等待增加时间边界
- `cancel(true)`会尝试以中断方式取消任务，`cancel(false)`则不会主动中断正在运行的任务

因此它只是把“等待结果”推迟到了真正需要结果的时候，并没有让等待本身消失。
...
`FutureTask`同时具备`Runnable`和`Future`两种身份：既可以被线程或线程池当作任务执行，也可以被外部拿来查询状态、取消任务或获取结果，因此经常被用来桥接“任务执行”和“结果获取”。
...
主动放弃占用的CPU资源，进入`TIMED_WAITING`状态，并进入休眠状态`mills`毫秒。当超过`mills`后，线程就会进入`RUNNABLE`状态，等待执行。
...
如果该线程处于`BLOCKED、WAITING、TIMED_WAITING`状态，执行`interrupt()`会抛出`InterruptedException`异常。
...
- `interrupt()`：实例方法，用于给目标线程发出中断信号。
- `interrupted()`：静态方法，检查**当前线程**是否中断，并会清除中断标记。
- `isInterrupted()`：实例方法，检查目标线程是否中断，但**不会**清除中断标记。

因此中断本质上是一种协作机制，而不是强制结束线程。线程收到中断信号之后，是退出、清理资源还是继续执行，取决于代码本身如何处理中断。

这些常见方法可以放在一起理解：

- `sleep()`：让当前线程进入`TIMED_WAITING`，**不会释放锁**
- `join()`：让当前线程等待目标线程结束
- `wait()`：必须在持有监视器锁时调用，并且会释放当前监视器
- `yield()`：只是提示调度器愿意让出CPU，不保证一定成功
- `park()`：基于许可机制阻塞线程，常用于更底层的并发组件
...
**不可变的对象一定是线程安全的。**无论是对象的方法实现还是方法的调用者，都不需要进行任何线程安全保障措施。
...
例如`ThreadLocal`、不可变对象以及只在当前线程内部可见的局部变量，本质上都属于“通过缩小共享范围来避免同步”的思路。



### 线程间通信

线程间通信的本质，是让多个线程围绕某个共享状态建立可见性和执行时序上的协作关系。常见方式有：

- `wait()/notify()/notifyAll()`：基于对象监视器进行等待与唤醒，必须配合`synchronized`使用
- `LockSupport.park()/unpark()`：基于许可机制阻塞和唤醒线程，是很多并发组件的基础
- `Condition.await()/signal()`：配合`Lock`使用，可以基于同一把锁拆分多个条件队列
- `volatile`共享变量：适合轻量级状态通知，例如某个开关位或结束标记

真正可靠的线程通信，通常都不是“只有一个通知动作”，而是建立在**共享状态 + 条件判断 + 等待/唤醒机制**三者配合之上。



### *线程池

线程池的核心目标，是把线程的创建、复用、调度和回收统一管理起来，避免频繁`new Thread()`带来的资源浪费和失控增长。

理解线程池时，最关键的几个参数包括：

- `corePoolSize`：常态下保留的并发能力
- `maximumPoolSize`：高峰时最多还能扩到多少线程
- `workQueue`：任务更倾向排队还是推动线程扩容
- `RejectedExecutionHandler`：系统超载时如何处理新任务

线程池并不是“线程越多越快”，而是在吞吐、延迟、资源占用和风险控制之间做权衡。更完整的线程池原理、参数和源码分析可以看：
...
- 优先减少共享可变状态，能不共享就不共享
- 优先使用不可变对象、并发容器、线程池、锁、条件队列等高层并发工具
- 线程中断要有明确策略，不要随意吞掉中断信号
- 线程池要有边界、有监控、能关闭，避免线程和任务无限堆积
- 遇到并发问题时，先保证正确性，再考虑性能优化

如果按学习路径来整理，这篇内容比较适合作为一条总线来读：

`基础概念 -> 线程状态与常见方法 -> 线程安全 -> 线程间通信 -> 线程池 -> 锁优化`

## 锁优化

JVM为了降低同步带来的性能损耗，在不同竞争强度下会采用不同的优化策略，常见包括：

- 自旋锁与适应性自旋：在短时间竞争场景下先尝试忙等，避免立刻阻塞挂起
- 锁消除：通过逃逸分析证明对象不会被其他线程访问时，去掉不必要的同步
- 锁粗化：把一串零碎的小同步块合并，减少频繁加锁解锁的成本
- 轻量级锁：在低竞争下尽量避免直接走重量级互斥量
- 偏向锁：针对长期被同一线程反复获取的锁进一步降低成本

更完整的锁优化与线程安全分析可以看：

{% post_link JVM相关及其拓展-七 %}
