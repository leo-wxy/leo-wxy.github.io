...
- `HashMap`：非线程安全，适合单线程或外部已经做好同步的场景
- `Hashtable`：线程安全，但基本是整表同步，并发性能较低
- `ConcurrentHashMap`：通过更细粒度的并发控制提升吞吐，更适合高并发访问场景
...
>
> 更准确地说，容量选择为2的幂，是为了让`(n - 1) & hash`既能高效替代取模，又能尽量把低位信息完整映射到桶索引上。如果容量不是2的幂，这种掩码方式就无法稳定覆盖所有低位组合，分布效果也会变差。
...
`UNTREEIFY_THRESHOLD`：当红黑树节点小于该值时，红黑树会转换回链表。发生在`resize()`扩容时。

`MIN_TREEIFY_CAPACITY`：当哈希表中的容量大于该值时，才允许链表转换红黑树。

这几个阈值可以放在一起理解：

- `8`并不意味着“只要链表一长就立刻树化”，而是认为冲突已经开始明显影响查询效率
- `6`是反树化阈值，给链表和红黑树之间留出一个缓冲区，避免在边界附近频繁来回切换
- `64`则说明：当数组容量还比较小时，冲突更可能是“桶太少”导致的，这时优先扩容通常比过早树化更划算
...
- `hash()`：将`key`转化成`hash值`。通过`扰动函数`生成对应`hash值`。
  ...
  之所以只把高16位再折叠到低16位，是因为HashMap最终计算桶索引时主要依赖低位：`(n - 1) & hash`。如果某些key的低位分布本身不好，那么单看原始`hashCode()`低位就容易把数据集中到少数桶里。把高位信息混进低位之后，可以让低位分布更均匀一些。
...
- `putVal()`：添加`key-value`的实际方法
  ...
  6. 如果是“找到相同key并覆盖旧值”，那么这是一次更新操作，不会增加`size`；只有真正插入了新节点，才会让`size`增加，并继续参与扩容判断。
  7. 插入成功后，在判断当前存储的键值对数量是否大于 `threshold阈值`，大于则触发扩容`resize()`操作。
...
由于我们使用的2次幂的扩展(*每次扩容为原大小的2倍*)，所以元素在扩容后数组的位置要不在原位置（`index`），要不就在原位置加上扩容前的数组长度(`index + oldCap`)。

其根本原因在于：容量翻倍之后，决定元素去向的只是新增出来的那一位二进制位。也就是说，判断`(hash & oldCap)`的结果是`0`还是`1`，就能知道节点到底是留在原桶，还是移动到`原索引 + oldCap`的位置，而不需要重新做一整套复杂计算。
...
- `ConcurrentModificationException`属于**快速失败(fail-fast)提示机制**，它主要在遍历期间检测到结构被并发修改时抛出异常
- 数据覆盖、数据丢失、扩容期间结构异常，则属于真正的并发安全问题，它们并不会总是通过异常“保护”你

- 当我们在对HashMap进行遍历时，如果在遍历期间我们对HashMap进行`put()、remove()`操作，会导致`modCount`发生变化(`expectedModCount != modCount`)，然后抛出`ConcurrentModificationException`异常，这就是**`fail-fast快速失败`**机制。
- 由于存在扩容机制，多线程操作HashMap时，调用`resize()`进行扩容可能会导致死循环的发生。

如果想要线程安全，还是推荐使用`ConcurrentHashMap`。

此外，虽然`HashMap`允许`null key`和`null value`，但也要理解它的语义边界：

- `null key`只能有一个，本质上会落在固定桶位
- `null value`也是允许的

因此在使用`get(key)`返回`null`时，要区分到底是“这个key不存在”，还是“这个key存在但value就是null”，必要时需要结合`containsKey()`一起判断。

### 使用HashMap时，使用什么对象作为key比较好？

**最好选择不可变对象作为key，因为为了计算`hashcode()`，就要防止键值改变，如果键值在放入时和获取时返回不同的hashcode，就会导致无法正确的找到对象。**

`String和Integer`等包装类就很适合作为key，而且`String`最常用。因为`String`是不可变的且`final`修饰(*保证key的不可更改性*)，并且已经重写了`equals()和hashcode()`方法(*不容易出现hash值的计算错误*)。
...
更严格地说，作为HashMap的key，至少要满足下面几点：

- 如果两个对象`equals()`相等，那么它们的`hashCode()`必须相等
- 最好不要让参与`equals()/hashCode()`计算的字段在放入Map之后继续变化
- 能设计成不可变对象时，尽量设计成不可变对象

否则对象放入HashMap之后，如果其哈希相关字段被修改，就可能出现“对象明明在Map里，却再也找不回来”的问题。
...
如果遍历时既需要`key`又需要`value`，一般优先使用`entrySet()`。因为`keySet() + get()`的方式会在遍历key之后，再根据key做一次查找，等于多走了一次定位流程。
