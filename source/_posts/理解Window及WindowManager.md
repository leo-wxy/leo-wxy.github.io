...
`Window`：它是一个抽象类，具体实现类为`PhoneWindow`，负责对View进行管理。每个Window都会对应一个`View`和一个`ViewRootImpl`，Window通过`ViewRootImpl`与View建立联系。
...
- **System Window(系统窗口)**：需要声明权限才能创建的Window，例如*TYPE_APPLICATION_OVERLAY*   `层级范围为2000~2999`



Window是分层的，层级大的Window会覆盖在层级小的Window上面，上面描述的层级范围对应的是`WindowManager.LayoutParams的type参数`。

通过`layoutParams.type = LayoutParams.TYPE_XXX`可以设置层级。创建系统悬浮窗通常需要声明`<uses-permission android:name="android.permission.SYSTEM_ALERT_WINDOW"/>`并通过系统授权。

> Android 8.0(API 26)及以上推荐使用`TYPE_APPLICATION_OVERLAY`，旧类型如`TYPE_PHONE/TYPE_SYSTEM_ALERT`已不建议继续使用。
...
| FLAG_NOT_TOUCH_MODAL  | 系统会将当前区域以外的触摸事件向下传递，Window以内的事件自己处理。*一般需要开启，否则其他Window无法接收事件。* |
...
> 主链路：`WindowManagerImpl.addView() -> WindowManagerGlobal.addView() -> ViewRootImpl.setView() -> IWindowSession.addToDisplay() -> WMS.addWindow()`
...
`mWindowSession`的类型是`IWindowSession`，它是一个Binder对象，用于进行进程间通信，也是`Session`在客户端的代理。

添加完成后，需要通过返回值`res`来判断是否添加成功。若是`WindowManagerGlobal.ADD_OKAY`说明添加成功。
...
> 主链路：`WindowManagerImpl.updateViewLayout() -> WindowManagerGlobal.updateViewLayout() -> ViewRootImpl.setLayoutParams() -> IWindowSession.relayout() -> WMS.relayoutWindow()`
...
`mWindow`即`W extends IWindow.Stub`，发送给WindowManagerService后用于接收WMS回调。
...
> 主链路：`WindowManagerImpl.removeView() -> WindowManagerGlobal.removeViewLocked() -> ViewRootImpl.die()/doDie() -> IWindowSession.remove() -> WMS.removeWindow()`
...
>
> **在上述三大操作中，虽然说是由`WindowManagerGlobal`去实现，但内部是依靠的`ViewRootImpl`，实际执行的是`WindowManagerService`。**
