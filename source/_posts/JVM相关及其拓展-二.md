...
- `加载`：把类的二进制数据引入虚拟机，并生成`Class`对象入口。
- `连接`：把类从“可读”进一步处理成“可校验、可分配、可解析”的状态。
- `初始化`：真正执行类变量赋值和静态代码块逻辑。
- `使用`：类已经完成初始化，可以被正常实例化、调用和访问。
- `卸载`：类及其加载器满足条件后，相关元数据才可能被回收。
...
- 通过子类去访问父类中定义的静态字段，通常只会触发父类初始化。
- 通过`SomeClass[]`这种数组定义引用类，不会触发`SomeClass`初始化。
- `static final`且在编译期可确定的常量，很多时候会在调用方常量池中直接展开，也不会触发目标类初始化。
...
也就是说，类的来源并不局限于磁盘上的`.class`文件，还可能来自：
...
这也是自定义类加载器、动态代理、热修复、模块隔离等技术能够成立的基础。
...
- 准备阶段：给类变量分配内存，并设置“默认零值”。
- 特殊情况下，带有`ConstantValue`属性的编译期常量，会在准备阶段直接得到指定值。
- 初始化阶段：执行`<clinit>()`，再按照程序语义完成静态变量赋值和静态代码块逻辑。

所以“准备阶段”和“初始化阶段”并不是同一件事：前者偏向内存与默认值准备，后者才真正开始执行类级别的Java代码。
...
在实际业务开发中，大多数类往往并不会轻易被卸载，原因通常不是“JVM不能卸载”，而是：

- 它们通常由应用类加载器长期加载并持有。
- 类加载器本身生命周期很长，导致相关`Class`对象始终可达。
- 业务代码里也很少主动去构造“可卸载的类加载器边界”。

因此真正更容易遇到类卸载场景的，通常是热部署、插件化、脚本引擎、容器隔离等需要动态加载/隔离类的系统。
