...
判断对象是否可回收时，一个更准确的说法是：**关键不在于“有没有变量名指向它”，而在于“能否从GC Roots沿引用链走到它”。**

另外还要结合引用强度来理解：

- 强引用：只要强引用还在，通常就不会被回收。
- 软引用：内存紧张时更容易被回收。
- 弱引用：下一次垃圾收集时通常就会被回收。
- 虚引用：主要用于跟踪对象被回收的时机，本身不影响对象存活。
...
补充：`finalize()`在现代Java实践里已经不推荐作为资源回收手段。

- 它执行时机不确定，无法保证及时释放资源。
- 可能拖慢对象回收，增加GC负担。
- 对象还可能在`finalize()`里“自救”，使生命周期更难推断。

实际开发里更推荐显式关闭资源、`try-with-resources`，或者使用更可控的清理机制，而不要把资源释放逻辑寄托在`finalize()`上。
...
这背后的核心原因在于不同代的对象“存活特征”不同：

- 新生代对象通常朝生夕灭，回收时大多数对象都能直接死掉，因此复制算法成本相对可控。
- 老年代对象存活率更高，如果还使用复制算法，复制成本和额外保留空间的代价都会明显升高。

所以并不是某种算法“天然最好”，而是要结合对象存活率、内存碎片、停顿时间这些因素去看它适不适合当前代。
...
从整体上看，收集器选择本质是在几个目标之间做权衡：

- 停顿时间：是否希望单次GC暂停尽量短。
- 吞吐量：是否更关注单位时间内用户代码执行占比。
- 内存占用与碎片：是否能接受额外内存开销、是否容易出现碎片。

可以粗略理解为：

- `Serial`偏简单直接。
- `Parallel Scavenge/Parallel Old`偏吞吐量优先。
- `CMS`偏低停顿，但要承担浮动垃圾和碎片问题。
- `G1`偏整体均衡与可预测停顿。
...
- 新对象通常优先分配在`Eden`
- 经过一次或多次Minor GC后仍存活的对象，会在`Survivor`区之间复制
- 年龄达到阈值，或者满足动态年龄判定时，对象会晋升到老年代
- 大对象、以及Survivor放不下的对象，可能更早进入老年代
...
- 可能是年龄自然增长后晋升
- 可能是大对象直接分配到老年代
- 也可能是Minor GC时Survivor空间不足，触发分配担保提前进入老年代

理解这条流转路径之后，再去看Minor GC、Major/Full GC的触发条件，会更容易把“对象分配”和“垃圾回收”串成一体。
